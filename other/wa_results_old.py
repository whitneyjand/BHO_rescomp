...
if __name__ == '__main__':
    ...
    def run_sims():
        ...
        def run_once(n_):
            ...
            rho = 5
            pthin = 0.96
            a = 0.3669613922269288
            g = 9.142233822830272
            s = 0.10183042009990073

            combo = 124750 #---- comb(n,2)
            ...
            # GET DIVERSITY SCORE
            res_signal = res.internal_state_response(tr, Utr, res.r0)
            res_diff = res_signal[1:] - res_signal[:-1]
            div = 0
            for i in range(n):
                div += np.sum(np.abs(np.subtract(res_diff, np.expand_dims(res_diff[:,i], axis=1))))

            div = div/combo
            div = div/len(res_diff)
            
            # GET VPT
            pred, p_sig = res.predict(ts, r0=res.r0, return_states=True)
            vpt = get_vptime(system, ts, Uts, pred, vpttol=0.6)

            ### CHANGEED - dt: from 0.01 to 0.001, vpttol: from 0.6 to 5, and duration: from 1000 to 40

            return (vpt, div)
        
        sims = bview.map_sync(run_once, range(1000))
        vpts = [s[0] for s in sims]
        divs = [s[1] for s in sims]
        print("vpts: ", vpts)
        print('\n############################################################################################')
        print('############################################################################################')
        print("\n\ndivs: ", divs)
        return np.mean(vpts), np.var(vpts), np.mean(divs), np.var(divs)
    

    sys.stdout = open(f'wa_results_{rho}_{pthin}.txt', 'a')
    avg_vpt, var_vpt, avg_div, var_div = run_sims()
    ...
    print('pthin: ',pthin)
    print('rho: ',rho,'\n')
    print('avg_vpt: ',avg_vpt)
    print('var_vpt: ',var_vpt,'\n')
    print('avg_div: ',avg_div)
    print('var_div: ',var_div)

    print('num_trials: 1000')
